<?php 
/**
 * Pengurus Page Controller
 * @category  Controller
 */
class PengurusController extends SecureController{
	function __construct(){
		parent::__construct();
		$this->tablename = "pengurus";
	}
	/**
     * List page records
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				pengurus.id_pengurus LIKE ? OR 
				pengurus.username LIKE ? OR 
				pengurus.email LIKE ? OR 
			...
			$this->view->search_template = "pengurus/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("pengurus.id_pengurus", ORDER_TYPE);
		}
		if($fieldname){
		...
		$page_title = $this->view->page_title = "Pengurus";
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("pengurus.id_pengurus", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$this->write_to_log("view", "true");
			$page_title = $this->view->page_title = "View  Pengurus";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		...
		return $this->render_view("pengurus/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		...
		$page_title = $this->view->page_title = "Add New Pengurus";
		$this->render_view("pengurus/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id_pengurus","username","email","password","jabatan","photo");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$cpassword = $postdata['confirm_password'];
			$password = $postdata['password'];
			...
			$modeldata['password'] = $this->modeldata['password'] = password_hash($password_text , PASSWORD_DEFAULT);
			if($this->validated()){
				$db->where("pengurus.id_pengurus", $rec_id);;
		...
		$page_title = $this->view->page_title = "Edit  Pengurus";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("pengurus/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
	...
	function delete($rec_id = null){
		...
		$db->where("pengurus.id_pengurus", $arr_rec_id, "in");
