<footer class="footer border-top">
	<div class="container-fluid">
		<div class="row">
			<div class="col-sm-4">
				<div class="copyright">&copy; <?php echo SITE_NAME ?> - <?php echo date('Y') ?></div>
			</div>
			<div class="col">
				<div class="footer-links text-right">
					<a href="<?php print_link('info/about'); ?>">Tentang Kami</a> | 
					<!--<a href="<?php print_link('info/help'); ?>">Help and FAQ</a> |-->
					<a href="<?php print_link('info/contact'); ?>">Kontak Kami</a>
					<!--<a href="<?php print_link('info/privacy_policy'); ?>">Privacy Policy</a> |-->
					<!--<a href="<?php print_link('info/terms_and_conditions'); ?>">Terms and Conditions</a>-->
				</div>
			</div>
			
		</div>
	</div>
</footer>